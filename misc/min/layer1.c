...
#define FRAME_LENGTH_MASK               (0xffU)
...
void min_rx_byte(uint8_t byte)
{
	...
	switch(rx_frame_state) {
        ...
        case RECEIVING_CHECKSUM_LOW:
            ...
            uint16_t fletcher16_rx_checksum=fletcher16_rx_finalize();
            if(rx_frame_checksum != fletcher16_rx_checksum) {
//            	printf("CHECKSUM FAIL %x(%d) != %x(%d)\n",rx_frame_checksum,rx_frame_checksum,fletcher16_rx_checksum,fletcher16_rx_checksum);
...
//		printf("%d < %d\n",tx_space,MAX_FRAME_SIZE);
//		return;
//	}
	// too much data
	if(control > MAX_FRAME_PAYLOAD_SIZE)
		return;
